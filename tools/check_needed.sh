...
if [ "$key" = win ]; then
    ...
    # the system DLLs a MinGW program may import, and the launcher's SDL2.dll
    allowed='^(kernel32|user32|gdi32|winmm|imm32|ole32|oleaut32|shell32|version|setupapi|advapi32|msvcrt|ucrtbase|ws2_32|iphlpapi|cfgmgr32|api-ms-win-.*|sdl2)\.dll$'
